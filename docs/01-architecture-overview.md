...
| コンポーネント | 技術 | 説明 |
|---|---|---|
| **Blog CRUD API** | Workers (Remix loader/action) | 記事の CRUD 操作（下書き保存・公開） |
...
# API（Remix loader/action で実装、独立 API エンドポイントなし）
# 画像アップロード: portal.edit.$id の action 内で R2 に保存
```
...
| **AI モデル** | Meta Llama 3.1 70B Instruct | Workers AI 経由 |
