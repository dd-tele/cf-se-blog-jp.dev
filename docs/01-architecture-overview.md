...
│  │  ┌──────────────────┐  ┌────────────────────────────────┐  │     │
│  │  │  Email Routing   │  │  cf-se-blog-email-worker       │  │     │
│  │  │  (宛先アドレス    │  │  (send_email binding で       │  │     │
│  │  │   検証・管理)    │  │   承認通知メール送信)          │  │     │
│  │  └──────────────────┘  └────────────────────────────────┘  │     │
│  └─────────────────────────────────────────────────────────────┘     │
└─────────────────────────────────────────────────────────────────────┘
...
| **API保護** | API Shield (Schema Validation) | OpenAPI 3.0 スキーマで全エンドポイントのメソッド・パス・ボディを検証 |
...
https://cf-se-blog-jp.dev/authors/:id               → 著者公開プロフィール
https://cf-se-blog-jp.dev/apply                      → 投稿者申請フォーム
...
https://cf-se-blog-jp.dev/portal/profile             → プロフィール編集（アバタークロップ含む）
https://cf-se-blog-jp.dev/portal/template-api        → Template API ガイド & API キー管理
...
https://cf-se-blog-jp.dev/admin/access-requests      → 投稿者申請管理（承認/却下/削除）
https://cf-se-blog-jp.dev/admin/qa                   → Q&A 管理
https://cf-se-blog-jp.dev/admin/users/:id            → ユーザー編集（アバタークロップ含む）
https://cf-se-blog-jp.dev/admin/presentation         → プレゼンテーション（管理者専用）
...
# GET  /api/v1/templates    → テンプレート一覧（Bearer/Cookie 認証）
# GET  /api/v1/templates/:id→ テンプレート詳細
# *    /api/v1/api-keys     → API キー管理 CRUD
...
| **メール通知** | Cloudflare Email Workers + Email Routing | 専用 Worker (cf-se-blog-email-worker) 経由 |
| **デプロイ** | Wrangler CLI + GitHub Actions | Pages 自動デプロイ |
