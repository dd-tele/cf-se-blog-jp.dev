...
### 2.3 Email Worker (cf-se-blog-email-worker)
| 項目 | 詳細 |
|---|---|
| **用途** | 承認通知メールの送信 |
| **バインディング** | `send_email` (Email Routing 経由でメール送信) |
| **送信元** | `noreply@cf-se-blog-jp.dev` |
| **呼び出し元** | Pages アプリから HTTP fetch で呼び出し |
| **ソース** | `workers/email-worker/` |

**アーキテクチャ:**
```
Pages (cf-se-blog)
  │  fetch(EMAIL_WORKER_URL, { method: "POST", body: { to, subject, text, html } })
  └─→ Worker (cf-se-blog-email-worker)
       │  send_email binding
       └─→ Email Routing → 宛先メールボックス
```

> **注:** Pages は `send_email` バインディング非対応のため、専用 Worker を経由してメールを送信。
> `send_email` は Email Routing で検証済みの宛先アドレスにのみ送信可能。
> 申請フォーム送信時に Email Routing API で宛先アドレスを自動登録し、ユーザーに検証を促す。
...
| **用途** | メインリレーショナルデータベース |
| **DB名** | `cf-se-blog-db` |
| **データ** | 記事、ユーザー、カテゴリ、テンプレート、AI ドラフトリクエスト、Q&A、監査ログ、投稿者申請 |
| **バックアップ** | D1 自動バックアップ + Time Travel（30日） |

**主要テーブル:**
- `users` — ユーザー情報・ロール（nickname, furigana, company, job_role, expertise 等のプロフィール情報含む）
- `access_requests` — 投稿者申請（status: pending / approved / rejected）
...
| 項目 | 詳細 |
|---|---|
| **用途** | メール送信用の宛先アドレス検証・管理 |
| **ドメイン** | `cf-se-blog-jp.dev` |
| **機能** | 宛先アドレスの登録・検証・削除（API 経由） |

**ライフサイクル:**
1. 申請フォーム送信時: `POST /accounts/{id}/email/routing/addresses` で宛先登録 → Cloudflare が検証メール送信
2. ユーザーが検証リンクをクリック → アドレスが「検証済み」に
3. 管理者が承認時: Email Worker 経由で通知メール送信
4. ユーザー削除時: `DELETE /accounts/{id}/email/routing/addresses/{tag}` で宛先削除
