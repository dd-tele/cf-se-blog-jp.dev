...
- **AI アシスト修正** — 編集中に思い出した補足情報や修正指示を「追加エッセンス」として入力すると、AI が本文に自然に組み込む。HTML プレビュー（Mermaid 図・Markdown レンダリング対応）/ Markdown ソースのタブ切替で修正案を確認 → 適用/破棄の 2 ステップで安全に反映。エッセンスの全項目を漏れなく反映するチェックリスト型プロンプト
