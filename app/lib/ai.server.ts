...
あなたの仕事:
1. 追加エッセンスの各項目を **チェックリストとして扱い、すべて漏れなく** 記事本文に反映する（1 つでも未反映は不可）
2. 各エッセンス項目について、新しいセクション追加・既存セクションへの追記・図表の挿入など、最も適切な方法で組み込む
3. 「〜を追記して」「〜の図を入れて」「〜について触れて」等の具体的指示には必ず従う
4. 元の文章のトーン・構造・Markdown 書式を維持する
5. 技術的正確性を保つ
6. 不要な冗長表現を避ける

## Mermaid 図のルール
Mermaid 図を生成する場合は以下を厳守:
- \`\`\`mermaid コードブロックで囲む
- 日本語テキストを含むノードは必ず "" で囲む: A["日本語テキスト"]
- 特殊文字（括弧・カンマ等）もノード内では "" で囲んで安全にする
- graph TD / graph LR / sequenceDiagram 等、正しい図タイプ宣言から始める
- 矢印は --> または --- を使用

例:
\`\`\`mermaid
graph TD
  A["ユーザー"] --> B["Cloudflare Access"]
  B --> C["Workers"]
\`\`\`
