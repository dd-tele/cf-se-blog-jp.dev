...
export const meta: MetaFunction = () => [
  { title: "プレゼンテーション — Cloudflare フィールドノート" },
];
  ...
  slides.push(
    <div key="tech" className="mx-auto max-w-7xl px-8 py-8 sm:px-12 sm:py-10">
      ...
      <div className="space-y-4">
        ...
        <TechDetail title="AI チャット Q&A — RAG + SSE ストリーミング + 多層防御" problem="記事ページで読者が質問できるチャットボットを実装したいが、RAG 精度・リアルタイム応答・Bot 攻撃対策・不適切コンテンツ対策を同時に実現する必要がある" solutions={["RAG: 記事本文 8K 文字 + Vectorize (bge-base-en-v1.5 埋め込み → topK:3 類似記事) でコンテキストを構築。記事優先 → Cloudflare 全般知識で補足 → 公式ドキュメント誘導の 3 段階回答戦略", "SSE ストリーミング: Hono の streamSSE ヘルパーで Workers AI の stream: true レスポンスをリアルタイム中継。ストリーム中断・空レスポンス（ガードレールブロック）の検知と error イベント送信", "多層防御パイプライン: Turnstile invisible → スパムパターン検出 → KV ベースレート制限 (10msg/min) → Llama Guard 3 8B モデレーション → AI Gateway ガードレール", "Llama Guard Fail-open 設計: モデレーションエラー時はブロックせず通過（可用性優先）。フラグ付きメッセージは DB に証跡保存、管理画面で確認可能", "会話履歴: 直近 10 メッセージを AI コンテキストに含め文脈維持。スレッド単位で 24h TTL 自動クリーンアップ（日次 opportunistic 削除）", "SE/Admin 手動回答: 管理画面からスレッドに直接回答を投稿可能。AI 回答では不十分な専門的質問に対応"]} files={["app/lib/chat.server.ts", "app/api/routes/chat.ts", "app/components/ChatWidget.tsx"]} color="blue" />
        <TechDetail title="AI テンプレートドラフト生成 — プロンプトエンジニアリング & 品質制御" problem="SE がメモ書きレベルの入力から企業導入事例スタイルのテックブログ記事を自動生成したいが、AI の出力品質（文体・構成・多言語混入・重複表現）にばらつきがある" solutions={["スタイルプリアンブル (約3,000文字): 企業 SE 視点の導入事例文体を強制。営業トーン・初心者日記トーンの禁止表現リスト、推奨表現・常套句リストを明記", "品質ガードレール 7 項目: 日本語のみ出力（非日本語混入禁止）、セクション冒頭の遷移表現多様化、セクション間の重複禁止、考慮だけで終わらせない深掘り指示、各セクション末尾にまとめ・考察・展望を追加", "文章構成指示: 箇条書き入力を鵜呑みにせず『行間を読んで文脈を補完』する指示。入力の事実は省略禁止。箇条書き/散文の使い分けルール", "Mermaid 図表の積極活用: flowchart, sequenceDiagram, erDiagram, gantt を場面に応じて自動挿入する指示", "テンプレート別プロンプト: 6 種のテンプレート（Zero Trust, Workers/Pages, Performance, Security, Network, Tips）ごとに専用 aiPromptTemplate を DB 管理。スタイルプリアンブル + テンプレート固有指示の 2 層構成", "ドラフトリクエスト記録: ai_draft_requests テーブルにモデル・レイテンシ・トークン数・入力/出力を保存し品質分析に活用"]} files={["app/routes/portal.templates.$id.tsx", "app/lib/templates.server.ts"]} color="amber" />
